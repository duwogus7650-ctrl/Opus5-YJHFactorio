...
function refreshInsp() {
  ...
  if (sig !== lastInspSig) {
    ...
    if (e.type === 'chemplant') {
      // **다섯 갈래를 한 목록에 적는다.** 숫자를 라벨에 박아 두는 것은 의도다 —
      // 무엇이 유리한지 도움말을 뒤지지 않고 고르는 자리에서 바로 견줄 수 있어야 한다.
      var chemOpts = [['plastic', '플라스틱 (가스 ' + SPEC.chemGasPerPlastic + ' → 1개/s)'],
                      ...
                      ['fuel-light', '고체 연료 (경유 ' + SPEC.lightPerFuel + ' → ' +
                                     SPEC.chemFuelLightRate + '개/s · 가스보다 2배 낫다)'],
                      ['crack-heavy', '중유 분해 (중유 ' + SPEC.crackHeavyIn + ' → 경유 ' +
                                      SPEC.crackHeavyOut + '/s)'],
                      ['crack-light', '경유 분해 (경유 ' + SPEC.crackLightIn + ' → 가스 ' +
                                      SPEC.crackLightOut + '/s)']];
    ...
    if (e.logicForced) {
      // **누가 잡고 있는지 말한다.** '지배 중' 만으로는 지도 어딘가의 제어기를 찾아
      // 헤매게 된다 — 회로가 여럿이면 그 탐색이 이 게임에서 제일 지치는 일이다.
      // axisBy 에 축마다 제어기 id 가 이미 적혀 있다. 꺼내서 이름과 길을 준다.
      // 축 이름은 이미 한국어다(35_logic 의 noteAxisWriter) — 영어 키로 매핑하려다
      // 하나도 못 찾았다. 있는 것을 그대로 쓰고, 설명만 덧붙인다.
      var AXIS_NAME = { '가동': '가동/정지', '게이트': '벨트 게이트', '필터': '인서터 필터',
                        '사격허가': '사격', '열차출발': '열차 출발' };
      var by = e.axisBy || {}, who = [], seen = {};
      for (var ax in by) {
        if (!by[ax]) continue;
        who.push((AXIS_NAME[ax] || ax) + ' ← <b>제어기 #' + by[ax] + '</b>');
        seen[by[ax]] = 1;
      }
      var ids = Object.keys(seen);
      c.push('<div class="ok" style="font-size:11px;margin-top:5px">⛓ 제어기 지배 중 — ' +
        (who.length ? who.join(' · ') : '축 미상') +
        '. 수동 조작은 지배가 풀린 뒤 적용된다' +
        (ids.length === 1
          ? ' <button id="gotoCtrl" data-id="' + ids[0] +
            '" style="min-height:32px;padding:2px 8px;margin-left:4px">보러 가기</button>'
          : '') +
        '</div>');
    }
    ...
    if (!fi.connected) {
      ...
      if (isOil || fi.oil > 0 || fi.gas > 0) {
        s.push('<div class="frow"><label>원유</label><span class="num" style="color:' +
          ...
          '</span></div>');
        // **중유·경유도 보여야 한다.** 정제소는 셋 중 하나만 차도 멈추는데, 안 보이면
        // '왜 멈췄나' 에 답할 길이 없다 — 멈춘 이유가 화면에 있어야 한다.
        s.push('<div class="frow"><label>중유</label><span class="num" style="color:' +
          (fi.cap > 0 && fi.heavy >= fi.cap - 0.5 ? 'var(--stop)'
           : (fi.heavy > 0 ? 'var(--run)' : 'var(--ink-2)')) + '">' + fmt(fi.heavy, 0) +
          ' / ' + fi.cap + (fi.cap > 0 && fi.heavy >= fi.cap - 0.5 ? ' · 가득 참' : '') +
          '</span></div>');
        s.push('<div class="frow"><label>경유</label><span class="num" style="color:' +
          (fi.cap > 0 && fi.light >= fi.cap - 0.5 ? 'var(--stop)'
           : (fi.light > 0 ? 'var(--run)' : 'var(--ink-2)')) + '">' + fmt(fi.light, 0) +
          ' / ' + fi.cap + (fi.cap > 0 && fi.light >= fi.cap - 0.5 ? ' · 가득 참' : '') +
          '</span></div>');
        s.push('<div class="frow"><label>석유가스</label><span class="num" style="color:' +
          (fi.cap > 0 && fi.gas >= fi.cap - 0.5 ? 'var(--stop)'
           : (fi.gas > 0 ? 'var(--run)' : 'var(--ink-2)')) + '">' + fmt(fi.gas, 0) +
          ' / ' + fi.cap + (fi.cap > 0 && fi.gas >= fi.cap - 0.5 ? ' · 가득 참' : '') +
          '</span></div>');
      }
    }
  }
...
function bindInspControls(e) {
  // 지배 중인 제어기로 데려간다 — 화면을 옮기고 그 회로를 연다.
  // **인스펙터가 다시 그려질 때마다 다시 걸어야 한다.** 처음엔 부팅 때 한 번 도는
  // bindSaveButtons 에 넣었다가, 그때는 버튼이 아직 없어서 아무 일도 안 했다.
  var gb = document.getElementById('gotoCtrl');
  if (gb) gb.onclick = function () {
    var cid = parseInt(gb.getAttribute('data-id'), 10);
    var ce = entities[cid];
    if (!ce) { toast('그 제어기는 이제 없다'); return; }
    cam.x = ce.tx + 1; cam.y = ce.ty + 1; clampCam();
    selected = cid; showInsp(ce);
    openLogic(ce);          // 편집기는 엔티티를 받는다 (id 를 넘기면 조용히 빈 화면이 된다)
  };
...
function fillHelp() {
  document.getElementById('helpBody').innerHTML = [
    ...
    '<p><b>그리고 노드마다 지난 8초의 파형이 깔린다.</b> 값 한 칸은 <b>지금</b>만 말하지만,',
    '회로가 왜 그렇게 굴었는지는 <b>방금 무슨 일이 있었는지</b>를 봐야 안다 —',
    '부하 차단이 발진하고 있는지, 래치가 언제 풀렸는지, 펄스가 실제로 나오고 있는지.',
    '파형은 <b>매 틱</b> 담으므로 1틱 펄스도 뾰족한 선으로 남는다(값창은 그것을 놓친다).',
    ...
    '<p><b>정제소는 하나를 셋으로 쪼갠다.</b> 원유 <b>20/s</b> 가 들어가면 <b>중유 6 · 경유 6 ·',
    '가스 8</b> 이 나온다 — 합은 그대로다(정제는 쪼개는 일이지 만드는 일이 아니다).',
    '화학공장은 <b>가스 10 → 플라스틱 1개/s</b>. 셋을 합치면 <b>720 kW</b> —',
    '증기기관 한 대를 거의 다 먹으니 발전을 먼저 늘려 두는 편이 낫다.</p>',
    '<p><b class="bad">그리고 여기서 반드시 한 번 막힌다.</b> 정제소의 레시피는 쪼갤 수 없는',
    '한 덩어리라, <b>셋 중 한 출구만 차도 전부 선다.</b> 중유는 그대로는 쓸 데가 없어 제일',
    '먼저 쌓이는데, 그러면 <b>가스 자리가 텅 비어 있는데도 가스가 안 나온다</b> —',
    '플라스틱 라인이 왜 멈췄는지 알려면 중유를 봐야 한다. 인스펙터가 셋을 다 보여주고,',
    '가득 찬 것에는 <b class="bad">가득 참</b>이라고 적는다.</p>',
    '<p><b>막힌 것은 아래로 흘려보낸다 — 분해(cracking)다.</b> 화학공장이 <code>중유 4 → 경유 3</code>,',
    '<code>경유 3 → 가스 2</code> 로 바꾼다. <b>부피가 준다</b>(75% · 67%) — 공짜가 아니어서,',
    '"남으면 무조건 분해" 가 늘 옳지는 않다. 아래를 아무리 비워도 <b>위가 막혀 있으면 사슬은',
    '선다</b>: 경유만 흘려보내면 잠깐 돌다가 중유가 다시 차면서 멎는다.</p>',
    '<p><b>그래서 무엇을 얼마나 흘려보낼지가 회로의 일이 된다.</b> 늘 분해하도록 켜 두면',
    '가스가 넘쳐 이번엔 가스가 정제소를 막고, 안 켜 두면 중유가 막는다. <code>유체 잔량</code>',
    '노드에서 <b>중유·경유·가스 중 하나를 골라</b> %로 읽고, <code>SR 래치</code>로 여닫으면',
    ...
    '하고 싶은 일 카드의 <b>"증기가 마르기 전에 미리 끄기"</b>가 그 회로를 절반 채워서 열어 준다.</p>',
        '<p><b>이송 펌프</b>는 <b>두 유체망을 합치지 않고</b> 뒤쪽에서 앞쪽으로 200/s 옮긴다.',
    '파이프로 이으면 그 순간 한 망이 되어 "저쪽이 찰 때까지 이쪽을 비운다" 를 할 수 없다.',
    '두 망을 남남으로 두면 <b>언제 옮길지를 제어기가 정한다</b> — 방향이 있으니 놓을 때 <code>R</code>로 돌리고,',
    '제어기의 [가동/정지]를 물리면 그 자리에서 멈춘다. 전기를 쓴다(취수 펌프와 달리).</p>',
...
'<h4>신호를 다듬고, 나누고, 단계로 쪼갠다</h4>',
    ...
    '<p><b>최고·최저 기록</b>(논리 II)은 <b>이번 판에서 가장 높았던/낮았던 값</b>을 기억한다.',
    '완충을 얼마나 둘지는 결국 <b>최악치가 정한다</b> — "증기가 가장 낮았을 때 몇 %였나" 를 알면',
    '탱크를 더 둘지 판단할 수 있다. 순간값은 눈 깜짝할 사이에 지나가므로 회로가 대신 기억한다.',
    '리셋을 물리면 그 순간부터 다시 잰다(리셋이 값보다 우선한다).</p>',
    '<p><b>지속 조건</b>(논리 II)은 <b>조건이 N초 이상 계속돼야</b> 참이 된다.',
    '평활 필터와 자리가 다르다 — 평활은 값을 눅여 <b>모든 반응을 늦추는</b> 대신 진짜 변화도 늦게',
    '알리고, 지속은 값을 안 건드린 채 <b>짧은 튐만</b> 버린다. 3초를 넘기는 순간 바로 참이 되므로',
    '<b>늦추는 것이 아니라 확인하는 것</b>이다. 조건이 한 번 끊기면 시계는 0으로 돌아간다.',
    '문장에서도 기억 자리에서 <code>초 이상 계속되면</code> 으로 고를 수 있다.</p>',
    '<p><b>변화율</b>(논리 III)은 <b>얼마나 남았나가 아니라 얼마나 빨리 줄고 있나</b>를 잰다.',
    '완충(저장 탱크)이 커질수록 수위 자체는 느리게 움직여 신호로 약해진다 — 그때 쓰는 것이 기울기다.',
    '증기가 초당 12씩 줄고 3,000 남았다면 <b>250초 뒤에 마른다</b>. 그 나눗셈은 [계산] 노드가 하고,',
    '이 노드는 그 분모를 만든다. <code>평활 창 s</code>는 튀는 값을 눅이는 시간이고, 0으로 두면 날것이다.',
    '문장에서도 <code>초 창으로 잰 초당 변화</code> 로 고를 수 있다.</p>',
    ...
    '<p><b>기계가 왜 멈췄는지 찾는 법.</b> 제어기가 잡고 있는 기계를 누르면 창에',
    '<b>어느 축을 어느 제어기가</b> 잡았는지 뜬다(예: 가동/정지 ← 제어기 #72).',
    '<b>[보러 가기]</b> 를 누르면 그 제어기로 화면이 옮겨 가고 회로가 열린다 —',
    '지도를 뒤지며 찾아다니지 않아도 된다.</p>',
    '<p><b>채널에 이름을 붙여라.</b> A~H 는 이름이 없으면 하루 뒤의 자신도 못 읽는다 —',
    '[자재] 판의 <b>Bus 신호 버스</b>에서 이름을 치면 노드에서 고를 때도 <code>A — 증기%</code> 로 보인다.',
    '그 판은 채널마다 <b>지금 값</b>과 <b>쓰는 곳 → 읽는 곳</b> 수를 함께 적는다 —',
    '<b>쓰는 곳이 0 인데 읽고 있는 채널</b>은 그 자체로 고장이고, 값만 봐서는 그것을 못 본다.</p>',
