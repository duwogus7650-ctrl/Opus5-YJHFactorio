...
function applyUpdateNow() {
  // **캐시를 지우는 것만으로는 모자란다.** 브라우저의 HTTP 캐시(GitHub Pages 는
  // max-age=600)가 그대로라, 지운 뒤 다시 받아도 같은 옛 파일이 온다 — 실기기에서
  // 갱신을 눌러도 도장이 안 바뀐 이유가 이것이었다. 시험 서버가 캐시 헤더를 안 보내서
  // 이 실패가 시험에서는 안 났다(서버가 다르면 시험은 우리 서버를 잰다).
  //
  // 그래서 순서가 셋이다: 캐시를 비우고 → **HTTP 캐시를 건너뛰어**(cache:'reload')
  // 새 파일을 실제로 받아 두고 → 다시 연다.
  var done = function () { location.reload(); };
  var ctrl = ('serviceWorker' in navigator) ? navigator.serviceWorker.controller : null;
  if (!ctrl) {
    // 워커가 없으면 캐시만 지우고 다시 연다 — 이 경우엔 애초에 캐시가 우리 것이 아니다.
  ...
  // 워커에게 **HTTP 캐시를 건너뛰고 새로 받아 두라**고 시키고, 다 받으면 다시 연다.
  var fired = false;
  var go = function () { if (!fired) { fired = true; done(); } };
  var onMsg = function (ev) { if (ev.data && ev.data.a === 'refreshed') go(); };
  navigator.serviceWorker.addEventListener('message', onMsg);
  ctrl.postMessage({ q: 'refresh' });
  setTimeout(go, 6000);          // 응답이 없어도 6초 뒤에는 연다 — 멈춰 있는 것이 더 나쁘다
}
