...
var ADVANCED_STEPS = [
  ...
  {
    id: 'oil-rig',
    title: '캘 수 없는 자원을 뽑는다 — 원유',
    why: '여기까지의 자원은 전부 <b>캐서 상자에 담는</b> 것이었다. 원유는 다르다. 채광기를 세우면 ' +
         '거절당하고, 뽑아도 아이템이 아니라 <b>유체</b>라 벨트에 못 싣는다. 물·증기로 배운 배관을 ' +
         '여기서 처음으로 <b>안 쓸 수가 없다</b>.',
    how: [
      '<b>T</b>에서 <b>석유 처리</b>를 연구한다',
      '지도에서 <b>보랏빛 웅덩이</b>가 원유 광맥이다 — 스폰에서 30타일쯤 떨어져 있다',
      '그 위에 <b>펌프잭</b>을 놓는다 (3×3 이 광맥 안에 들어가야 선다)',
      '<b>파이프</b>로 <b>정제소</b>까지, 다시 파이프로 <b>화학공장</b>까지 <b>맞대어</b> 잇는다',
      '전주를 끌고 온다 — 셋을 합치면 <b>720 kW</b> 다 (증기기관 한 대를 거의 다 먹는다)',
      '화학공장에서 <b>플라스틱</b>이 나오면 통과다. 고체라 <b>인서터</b>로 빼낸다'
    ],
    need: '펌프잭 = 강철 5 + 톱니 10 + 회로 5 + 파이프 4 · 정제소 = 강철 10 + 회로 10 + 톱니 10 + 파이프 10 · ' +
          '화학공장 = 강철 5 + 회로 5 + 톱니 5 + 파이프 5 · 연구는 적색 60 + 녹색 60',
    check: function () {
      if ((inventory['plastic'] || 0) >= 1) return true;
      return countEntityWhere('chemplant', function (e) { return (e.out['plastic'] || 0) >= 1; }) >= 1;
    }
  },
  {
    id: 'oil-control',
    title: '막힌 것을 흘려보낸다 — 분해를 제어기에 맡긴다',
    // **이 단계는 한 번 낡았다.** 정제소가 가스 하나만 낼 때는 "가스가 넘치면 정제소를
    // 쉬게 한다" 가 맞았는데, 셋으로 쪼개진 뒤로는 실제로 막는 것이 중유·경유다.
    // 게임이 바뀌면 게임이 가르치는 것도 같이 바뀌어야 한다 — 안 그러면 튜토리얼이
    // 일어나지 않는 상황을 연습시키고, 늘 일어나는 상황은 안 가르친다.
    why: '정제소는 <b>중유·경유·가스를 한 덩어리로</b> 낸다. 한 출구만 차도 <b>전부 선다</b> — ' +
         '중유는 그대로는 쓸 데가 없어 제일 먼저 쌓이고, 그러면 <b>가스 자리가 텅 비어 있는데도</b> ' +
         '가스가 안 나온다. 그렇다고 분해를 늘 켜 두면 이번엔 가스가 넘쳐 같은 자리에서 막힌다. ' +
         '<b>언제 얼마나 흘려보낼지</b> — 이 게임이 계속 물어 온 그 질문의 석유판이다.',
    how: [
      '화학공장을 한 대 더 놓고 레시피를 <b>중유 분해</b>로 건다',
      '제어기를 <b>좌클릭</b>해 편집기를 연다',
      '입력에서 <b>유체 잔량</b>을 놓고, 대상은 <b>정제소</b>, 기름은 <b>중유</b>를 고른다',
      '<b>비교</b> 노드로 문턱을 잡는다 (예: <b>중유% &gt; 60</b>)',
      '그 결과를 <b>기계 가동/정지</b>에 잇고 대상을 <b>분해 화학공장</b>으로 한다',
      '읽으면 "<b>중유가 60%를 넘으면 분해를 켠다</b>" 가 된다',
      '떨리면 <b>SR 래치</b>로 문턱을 벌린다 — 부하 차단에서 한 것과 같은 손버릇이다'
    ],
    need: '<b>문장으로도 된다</b> — [문장으로] 화면에서 읽을 것을 <b>석유 잔량</b>으로 두고 ' +
          '기름을 고르면 같은 회로가 만들어진다 · 분해는 공짜가 아니다(중유 4 → 경유 3)',
    check: function () { return ctrlFeeds('fluid', ['enable']); }
  }
];
