...
var RECIPES = {
  ...
  // **석유의 두 번째 보상.** 탄창은 적이 올 때만 필요해서, 석유 사슬(건물 3대 + 720 kW)의
  // 값어치가 판마다 달랐다. 회로는 조립기·연구소·터렛까지 어디에나 들어가는 물건이라
  // 이쪽은 늘 이득이 된다.
  //   기본  전선 3 + 철판 1 → 회로 1   (개당 전선 3 · 철 1)
  //   이것  전선 3 + 플라스틱 1 → 회로 2 (개당 전선 1.5 · 철 0 · 플라스틱 0.5)
  // **속도는 같게 둔다**(둘 다 초당 2개). 이득은 재료여야지 처리량이면 안 된다 —
  // 처리량까지 주면 조립기 수가 반으로 줄어 전력·오염 계산이 통째로 흔들린다.
  'circuit-plastic': { cat: 'craft', time: 1.0, inp: { 'wire': 3, 'plastic': 1 },
                    out: { 'circuit': 2 }, handOk: true, tech: 'oil' }
};
...
var TECHS = {
  ...
  'logic-mem':  { name: '논리 II — 기억소자', cost: { 'sci-red': 40 }, needs: ['logistics'],
                  unlock: ['SR 래치', '카운터', '엣지 검출', '샘플 홀드', '최고·최저 기록', '지속 조건', '상태기계'],
  ...
  'oil':        { name: '석유 처리', cost: { 'sci-red': 60, 'sci-green': 60 }, needs: ['steel', 'logistics'],
                  unlock: ['펌프잭', '정제소', '화학공장', '탄창 레시피', '회로기판 레시피'],
                  desc: '원유를 뽑아 플라스틱까지. 플라스틱을 섞으면 탄창이 철판 4개에 2개 나오고, 회로는 전선 3개에 2개 나온다 — 철은 안 들고 구리는 절반이다.' },
