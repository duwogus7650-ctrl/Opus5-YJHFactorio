...
(function () {
  ...
  function finish() {
    ...
    // **솎았어도 그리기는 했는가.** 그림을 4프레임에 한 장으로 줄였다 — 이 주행이
    // 40분을 넘긴 원인이 렌더였기 때문이다(시뮬만이면 15초).
    //
    // 정직하게 적자면: **이 드라이버는 원래도 렌더러를 거의 안 밟는다.** 드라이버가
    // setTimeout 으로 시뮬을 직접 밀어서 rAF 가 몇 번 안 돌고, 헤드리스에서 그린
    // 장수는 솎기 전에도 십수 장 수준이다. 비용은 주행 중이 아니라 **드라이버가 끝난 뒤**
    // 브라우저가 가상시간 120초를 마저 렌더하는 구간에 있었다. 렌더러의 실제 검사는
    // driver/uismoke/mobile 이 한다. 그래서 이 게이트는 "한 장도 안 그리는 상태가
    // 되지는 않았다" 까지만 보증한다 — 그 이상을 주장하면 거짓말이 된다.
    var drewN = G.gfx().renders;
  ...
  function go() {
    try {
      ...
      G = window.__GAME;
      // 녹화가 아니면 아무도 이 판을 안 본다 — 그림을 4프레임에 한 장으로 솎는다.
      // 재는 값은 하나도 안 바뀐다(렌더는 상태를 안 건드린다). 헤드리스 Edge 는
      // 드라이버가 끝난 뒤에도 가상시간 120초를 마저 렌더하는데, 공장이 다 자란
      // 40분 판을 그 시간 내내 그리느라 **주행 하나가 40분을 넘겼다**(시뮬만이면 15초).
      // 솎기 배수는 4다 — 30으로 잡았더니 주행 내내 그린 그림이 1장뿐이라
      // 렌더러가 40분 중에 터져도 아무도 안 보게 됐다(실측: 솎기 없이 248장).
      // **여기서 부른다** — 파일 맨 위에서 부르면 그때 G 는 아직 undefined 다.
      if (DRIVE && G.renderThin && !qs.get('nothin')) G.renderThin(4);
      G.reset(SEED);
